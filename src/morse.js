...
function decodeMorse(morseCode) {
  ...
  var splitWords = morseCode.split(wordSpacing);

  //Loop through each split morse word
  splitWords.forEach((word)=>{
    var englishCharacterContainer = [];

    //Split each morse word into an array of morse characters
    var splitCharacters = word.split(characterSpacing);
    ...
    splitCharacters.forEach((morseCharacter)=>{
      ...
      if(MORSE_CODE.hasOwnProperty(morseCharacter)){
        var englishCharacter = MORSE_CODE[morseCharacter];
      } 
    ...
    var englishWord = englishCharacterContainer.join('');
